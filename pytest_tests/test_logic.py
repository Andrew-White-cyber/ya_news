...
def test_author_can_edit_comment(comment, news, updated_form_data, author_client):
    news_url = reverse('news:detail', args=(news.id,))
    url_to_comments = news_url + '#comments'
...
def test_user_cant_edit_comment_of_another_user(news, comment, not_author_client, updated_form_data, form_data):
    edit_url = reverse('news:edit', args=(comment.id,))
